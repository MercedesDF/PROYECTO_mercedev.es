...
17. **Trazabilidad Histórica Interna:** Todo documento (`.md`, `.txt`, etc.) y script (`.py`, `.js`, etc.) debe incluir siempre en su inicio un bloque de comentarios con el historial de modificaciones (indicando fecha y fase/épica). Esto permite una evaluación visual rápida de la vigencia del archivo y sienta la estructura de datos necesaria para que los linters (`merci-audit.py`) detecten automáticamente la "Deriva Documental" (Document Drift) entre el código y los manuales.
