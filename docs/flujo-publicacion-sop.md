...
2. **Curación Dinámica (Promote):** Ejecutar en la terminal:
   ...
   merci promote
   ```
   *Nota:* El asistente leerá el campo `tema` del YAML para enrutar mágicamente el archivo a `biblioteca/` o `art-de-cote/`. Al finalizar con éxito, **se preguntará si se desea invocar al Agente Blogger** para que genere automáticamente el post promocional en la incubadora.
...
### Paso a Paso:
1. **Incubación:** Nace de la llamada automática de `merci promote` (Agent Chaining) o invocando manualmente a `merci-blogger.py`. El Blogger genera el post en `incubacion/` con `estado: "incubacion"`, `tema: "Blog"` y `estado_social: "en_cola"`. Una vez revisado, se cambia el estado a `"borrador"` y el estado social a `"aprobado"`.
...
4. **Despliegue a Producción (Contenido Dinámico):** Adición de las variables `WP_PROD_URL`, `WP_PROD_USER` y `WP_PROD_APP_PASSWORD` al archivo `.env`. Al ejecutar `merci deploy` (o `merci completo`), el orquestador leerá estas variables en memoria y enviará los artículos a producción sin necesidad de modificar el archivo `.env` a mano, validando la caché multi-entorno automáticamente.
5. **Gobernanza del Buffer Social (LinkedIn):** La gestión de la cola de redes es puramente declarativa a través de los archivos Markdown:
   ...
   *   **Aprobar (Interactivo):** Ejecutar `merci linkedin`. El script muestra un menú numerado ordenado por fecha con los posts pendientes, permite elegir cuál revisar, y tras la confirmación (`s/N`) lo pasa a estado `"aprobado"`.
...
**Destino:** Repositorio público (`merci-boilerplate`) y Subdominio de demostración (`boilerplate.mercedev.es`).
