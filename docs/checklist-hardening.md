...
- [x] **Fronteras Inmutables (Nginx):**
  - El entorno dinámico (`/blog`) se sirve mediante un `alias` en Nginx desde un directorio físico separado (`/var/www/tu_wordpress`).
  - WordPress no tiene permisos de escritura sobre la raíz estática (`/var/www/tu_proyecto/public`).
